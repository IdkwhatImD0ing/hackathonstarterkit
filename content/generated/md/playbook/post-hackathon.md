...
One skill that runs the other four. It interviews you a single time, then generates the portfolio site, README, Devpost, and YouTube description in order, so they share one tagline, one set of stats, and one story. Best if you want everything done in a single sitting.
...
Installs Anthropic's frontend-design skill for a one-of-a-kind look, then builds a one-page case study (hero, problem, architecture, demo, results, team, honest limits). Works even for non-web projects, and never invents awards or stats.
...
Writes the demo video's title, description, tags, and chapter timestamps so recruiters can find it. Reads your repo, asks for the video beats, and never fakes a timestamp or award.
