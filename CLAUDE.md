...
- **Cheat sheet**: the during-the-hackathon prompt rack. Phases and paste-ready prompts live in `lib/cheat-sheet.ts`, rendered by `app/cheat-sheet/page.tsx` through `components/cheat-sheet-browser.tsx` (phase tabs, one prompt card at a time; every prompt still renders into the DOM behind `hidden`, per the SEO rule in `docs/page-content-conventions.md`). Three invariants for prompts: **self-collecting** (they interview the reader, never `[FILL THIS IN]` placeholders), **prerequisite-aware** (`needs` links to the prompt that produces the artifact, and the prompt text tells the agent to send the reader back), and **skill-first** (install and run a skill from `skills/` where one fits). It is deliberately **not** in the content registry (`lib/content-pages.ts`): no generated `.md`, no `<CopyForAi>`, listed in `MARKDOWN_EXCLUDED_PATHS`.
- **Shared agent prompts**: `lib/prompts/` holds **one file per prompt** offered in more than one place (skills install, the CLAUDE.md/AGENTS.md setup command, the README/Devpost/YouTube agent prompts), plus `skills.ts` for the shared helpers and `index.ts` as the barrel. Mirrors `lib/blog/posts/`. Import from `@/lib/prompts`, never from an individual file. Each prompt file's doc comment lists the pages that use it; keep that line current. A prompt used on exactly one page stays in that page file, and moves here the moment a second page needs the identical text.
...
- **Sitemap** (`app/sitemap.ts`) auto-pulls slugs from `lib/playbook.ts`, `lib/non-coder-sections.ts`, `lib/non-coder-skills.ts`, and `lib/blog.ts`. Data-driven routes appear automatically; **standalone routes (e.g. `app/cheat-sheet`, `app/terms`, `app/media-kit`) must be added to `app/sitemap.ts` manually.**
