...
- **Cheat sheet**: the during-the-hackathon prompt rack. Sections and paste-ready prompts live in `lib/cheat-sheet.ts`, rendered by `app/cheat-sheet/page.tsx` via `components/prompt-card.tsx`. It is deliberately **not** in the content registry (`lib/content-pages.ts`): no generated `.md`, no `<CopyForAi>`, listed in `MARKDOWN_EXCLUDED_PATHS`. Prompts use `[SCREAMING BRACKETS]` for placeholders; the card highlights them without changing the copied text.
