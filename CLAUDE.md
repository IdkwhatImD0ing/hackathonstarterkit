...
- **Shared agent prompts**: any prompt offered on two or more pages lives in `lib/agent-prompts.ts` (skills install, the CLAUDE.md/AGENTS.md setup command, the README and Devpost agent prompts). Page-specific prompts stay in the page file; move one into `lib/agent-prompts.ts` the moment a second page needs the same text.
